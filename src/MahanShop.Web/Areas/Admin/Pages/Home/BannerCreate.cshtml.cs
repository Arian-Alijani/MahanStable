...
/// <summary>ایجاد بنر میانی جدید + آپلود تصویر (دسکتاپ و اختیاری موبایل).</summary>
public class BannerCreateModel : PageModel
{
    private readonly IMediator _mediator;
    private readonly ImageUploadService _upload;
    public BannerCreateModel(IMediator mediator, ImageUploadService upload)
    {
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        try
        {
            await _mediator.Send(new CreateBannerCommand(
                Subtitle, img.WebPath!, mobileUrl, LinkUrl, DisplayOrder, IsActive));
            TempData["AdminOk"] = "بنر میانی ساخته شد.";
