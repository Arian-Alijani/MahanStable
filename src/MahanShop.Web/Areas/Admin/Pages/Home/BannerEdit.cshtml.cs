...
/// <summary>ویرایش بنر میانی. تصویر فقط در صورت آپلود جدید جایگزین می‌شود.</summary>
public class BannerEditModel : PageModel
{
    private readonly IMediator _mediator;
    private readonly ImageUploadService _upload;
    public BannerEditModel(IMediator mediator, ImageUploadService upload)
    {
    ...
    {
        var dto = await _mediator.Send(new GetBannerForEditQuery(Id));
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        try
        {
            await _mediator.Send(new UpdateBannerCommand(
                Id, Subtitle, ImageUrl ?? "", MobileImageUrl, LinkUrl, DisplayOrder, IsActive));
            TempData["AdminOk"] = "بنر میانی به‌روزرسانی شد.";
