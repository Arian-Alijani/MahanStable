...
- 2026-06-13: **بخش «گرید دسته‌بندی صفحهٔ اصلی» تکمیل + اصلاحات موبایل.** بک‌اند (Add/Remove/Move/Reorder/Settings commands + GetHomeCategoryBoard query + HomeCategoryStyle enum: Bento/Grid/Scroll/Pills) و UI ادمین (کارت‌های انتخاب سبک، لیست drag&drop + دکمه‌های بالا/پایین، چیپس‌های افزودن) از پیش در کامیت `a7cd4dd` ساخته شده بود؛ این سشن:
  (۱) باگ چک‌باکس «فعال‌بودن گرید» رفع شد — Razor `checked="@false"` همیشه تیک می‌خورد؛ با شرط `@(... ? "checked":"")` + `<input type=hidden value=false>` الگوی استاندارد چک‌باکس MVC اعمال شد تا خاموش‌کردن واقعاً پست شود.
  (۲) پالس بصری `just-moved` بعد از جابه‌جایی (درگ یا لمسی) برای بازخورد بهتر.
  (۳) **دراور موبایل**: قفلِ سختِ پنل به لبهٔ راستِ فیزیکی با `right:0 !important; left:auto !important` + `transform !important` تا هیچ قاعدهٔ RTL (بوت‌استرپ/UA) آن را به چپ نبرد (رفع گزارش «بعد از باز شدن می‌پرد چپ»). با probe در مرورگر تأیید شد: ورود از راست، استراحت روی راست.
  (۴) دکمهٔ خروج از حساب در هدر موبایل از قبل با `d-none d-lg-block` پنهان بود؛ بدون تغییر باقی ماند (خروج فقط از منوی همبرگری).
