...
> قواعد وابستگی مقدسن. وابستگی فقط رو به داخل. نقض = build رد.
...
- Application فقط Domain + interface (نه EF مستقیم؛ از `IApplicationDbContext`).
- Infra.Data پیاده‌سازی interfaceهای Application.
- Web فقط Controller/View — بدون business، فقط MediatR `Send`.
...
## قواعد ثابت
- ساختار Controller/Action عین نمونه (URL سازگار): `{controller=Home}/{action=Index}/{id?}`.
- Admin + PanelUser = Razor Pages.
- فروشگاه عمومی = MVC Controller + View.
