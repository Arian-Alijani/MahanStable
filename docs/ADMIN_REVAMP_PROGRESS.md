...
- **تصمیم‌های طراحی:**
  - **تب «ویژگی‌ها»:** همیشه نمایش داده می‌شود (نه فقط وقتی Features وجود دارد). badge شمارش ویژگی‌ها. اگر خالی بود → `pd-specs-empty` با پیام راهنما. آیکن SVG inline روی هر دو تب (معرفی + ویژگی‌ها).
  - **بخش اطلاعات بهبودیافته:** `pd-meta-row` برای برند+کد با آیکن. `pd-keyspecs` نمایش ۵ ویژگی کلیدی با dotted-line separator. دکمه «X ویژگی دیگر» → اسکرول smooth به تب ویژگی‌ها. `pd-stock-row` موجودی + تعداد variant‌ها. تگ‌ها → لینک‌های کاتالوگ.
  - **جعبه خرید:** نشان تخفیف درصدی (`pd-buy__discount-badge`) در کنار قیمت قدیمی. آیکن گارانتی به perks. طراحی قیمت تمیزتر.
  - **تأیید موارد قبلاً پیاده‌شده:** `product-variant.js` (device flow + simple flow) کامل و کارا بود. `DiscountPercent` روی `_ProductCard.cshtml` کامل بود. `pcard__badge`/`pcard__old` روی `site.css` کامل.
- **فایل‌های تغییریافته:**
  - `src/MahanShop.Web/Views/Catalog/Detail.cshtml` — بازنویسی کامل: تب‌بندی بهبودیافته + بخش اطلاعات + جعبه خرید.
  - `src/MahanShop.Web/wwwroot/css/product-detail.css` — بازنویسی کامل: کلاس‌های جدید F11 + hover effects + ریسپانسیو.
- **لمس فروشگاه عمومی (صریح):** `Views/Catalog/Detail.cshtml`, `wwwroot/css/product-detail.css` — مجاز طبق روادمپ §1 قانون ۲.
...
### F10 — داشبورد گرافیکی (آمار + نمودار SVG + سفارش‌های اخیر + موجودی کم) ✅ (2026-06-13)
- **چه شد:** صفحهٔ داشبورد ادمین از یک ساختار ساده (۷ کارت ساده + ۲ جدول) به یک داشبورد گرافیکی مدرن بازطراحی شد.
- **تصمیم‌های طراحی:**
  - **۴ کارت آماری اصلی گرافیکی:** فروش کل · کل سفارش‌ها · کل کاربران · محصولات فعال — هر کارت آیکن SVG رنگی، گرادیان مخصوص خود، subtitle با اطلاعات تکمیلی (مثلاً امروز/منتظر پردازش/موجودی کم)، و لینک‌پذیر به تب مربوطه. hover با transform/shadow.
  - **نمودار ۷ روزه SVG دستی (Domestic-only):** بدون lib خارجی. ترکیبی Bar (تعداد سفارش) + Line (مبلغ فروش) با grid lines، Y-axis labels، tooltip از طریق `<title>` تگ. روز جاری با رنگ پررنگ‌تر متمایز. viewBox واکنش‌گرا (`width:100%; max-height:260px`). نام روزها به فارسی (شنبه..جمعه).
  - **جداکنندهٔ روز:** بر مبنای `PaidAt` (تاریخ پرداخت) — هم‌خوان با منطق `TodaySales`. GroupBy روی `PaidAt.Value.Date` در DB. ۷ روز شامل امروز با حلقه `Range(0,7)` پر می‌شود (روزهای بدون فروش = 0).
  - **سفارش‌های اخیر بهبودیافته:** جدول با hover row، badge وضعیت (از `OrderStatusView`)، آیکن ساعت در سرتیتر، لینک «همه سفارش‌ها».
  - **موجودی کم:** جدول جداگانه با pill شمارش کل، badge مخصوص (is-low زرد / is-zero قرمز)، لینک «مدیریت موجودی».
  - **سربرگ:** عنوان + دکمه‌های اقدام سریع (مدیریت سفارش‌ها/مدیریت محصولات) با آیکن SVG.
  - **محاسبهٔ `TotalOrderCount` جدید:** کل سفارش‌ها به‌غیر از `Pending`/`AwaitingPayment` (هم‌سو با لیست سفارش‌های اخیر و گزارش F8).
- **فایل‌های تغییریافته:**
  - `src/MahanShop.Application/Features/Admin/Dashboard/GetDashboardStatsQuery.cs` — `TotalOrderCount` به DTO؛ `DashboardDailyPointDto` جدید (Date/OrderCount/Sales)؛ `Last7Days` به DTO؛ شمارش جدید + GroupBy روزانهٔ ۷ روز اخیر (پر‌کردن روزهای خالی با صفر).
  - `src/MahanShop.Web/Areas/Admin/Pages/Index.cshtml` — بازنویسی کامل: ۴ کارت گرافیکی، نمودار SVG ۷روزه (bar+line)، جدول سفارش‌های اخیر، جدول موجودی کم.
  - `src/MahanShop.Web/wwwroot/admin/admin.css` — بلوک `dash10-*` جدید (~۱۷۰ خط): head/stats/stat-card با آیکن گرادیان/chart-box/chart legend/grid panel/table/stock badge + ریسپانسیو ۳ breakpoint (1200/992/640).
- **نکتهٔ Razor:** المان SVG `<text>` در داخل `@for { ... }` با Razor compiler به‌عنوان literal-text-helper اشتباه گرفته می‌شود → باید با `<g>` wrap شود تا اولین تگ بعد از brace یک HTML element باشد (نه `<text>` تنها). ۳ مورد رفع شد.
- **لمس فروشگاه عمومی:** هیچ. فقط `Areas/Admin/Pages/Index.cshtml`, `Features/Admin/Dashboard/`, `wwwroot/admin/admin.css`.
- **migration:** هیچ. صفر تغییر Domain/DB. فقط Query و View/CSS.
- **build:** ✅ `dotnet build MahanShop.sln` = **0 Error** (5 warning همگی pre-existing در فایل‌های فاز‌های قبل). `bash tools/check-js.sh` = 8 ok / 10 tests passed. Domestic-only audit در F10 files = صفر URL خارجی.
- **قدم بعد = F11** (نمایش محصول عمومی: تب «ویژگی‌ها» کنار «معرفی» + انتخاب مدل/زیرمدل برای محصولات واریانتی + نمایش درصد تخفیف روی کارت‌های کاتالوگ/صفحهٔ اصلی). **⚠️ این فاز فروشگاه عمومی را لمس می‌کند** — در PROGRESS ثبت شود.
